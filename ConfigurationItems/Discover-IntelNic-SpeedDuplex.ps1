<#
Discover-IntelNic-SpeedDuplex.ps1

Discover if network adapter speed is configured at 100Mbps full duplex
Change the $Duplexvalue variable on lines 64 and 65 for other speed/ duplex settings
...
Output:
* 100Mbps full duplex enabled --> "Enabled"
* Other network speed duplex setting --> "Disabled"
...
Get-ChildItem -Path $RegPath -Exclude "Properties","Configuration"|ForEach {
    ...
    If ($Adapters.ContainsKey((Get-ItemProperty -Path Registry::$_ -name DriverDesc).DriverDesc)){
        ...
        If ((Get-ItemProperty -Path Registry::$_ -name $DuplexName).$DuplexName -ne $DuplexValue){
            # 100 Mbps full duplex is not enabled
            Write-Host "Disabled"
        }
        Else {
            # 100 Mbps full duplex is enabled
            Write-Host "Enabled"
        }
    }
}