<#
Remediate-IntelNic-LegacySwitchMode.ps1

Enable legacy switch mode if required
    ...
    If ($Adapters.ContainsKey((Get-ItemProperty -Path Registry::$_ -name DriverDesc).DriverDesc)){
        ...
        If ($SwitchModeName){
            ...
            If ((Get-ItemProperty -Path Registry::$_ -name $SwitchModeName).$SwitchModeName -ne $SwitchModeValue){
                ...
                # Enable legacy switch mode
                Set-ItemProperty -Path Registry::$_ -name $SwitchModeName -Value $SwitchModeValue
